...
const updatePointerType = <T extends string = PlaitPointerType>(
  board: PlaitBoard,
  pointer: T
) => {
...
function updateZoom(board: PlaitBoard, newZoom: number, isCenter = true) {
  ...
  if (
    ...
    focusPoint = [
      movingPoint[0] - nativeElementRect.x,
      movingPoint[1] - nativeElementRect.y,
    ];
  }
  ...
  const newOrigination = [
    centerX - focusPoint[0] / newZoom,
    centerY - focusPoint[1] / newZoom,
  ] as Point;
...
function fitViewport(board: PlaitBoard) {
  ...
  const newOrigination = [
    centerPoint[0] -
      boardContainerRect.width / 2 / newZoom +
      scrollBarWidth / 2 / zoom,
    centerPoint[1] -
      boardContainerRect.height / 2 / newZoom +
      scrollBarWidth / 2 / zoom,
  ] as Point;
  updateViewport(board, newOrigination, newZoom);
}
...
function fitViewportWidth(board: PlaitBoard, options: FitViewportOptions) {
  ...
  let finalHeight =
    elementHostBox.height * newZoom + 2 * options.autoFitPadding;
  if (finalHeight > options.limitHeight) {
    const containerEl = boardContainer.closest(
