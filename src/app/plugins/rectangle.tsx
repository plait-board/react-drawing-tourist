...
export const rectanglePlugin = (board: PlaitBoard) => {
    ...
    board.drawElement = (props: RenderElementProps) => {
        if (props.element.type === 'geometry' && props.element.shape === 'rectangle') {
            ...
            // const rectangleG = PlaitBoard.getRoughSVG(board).rectangle(start[0], start[1], width, height);
            // props.attributes.ref.current.append(rectangleG);
            console.log(props.attributes.ref.current);
            return <></>
        }
