...
export const RectangleClient = {
  isHit: (origin: RectangleClient, target: RectangleClient) => {
    return (
      RectangleClient.isHitX(origin, target) &&
      RectangleClient.isHitY(origin, target)
    );
  },
  ...
  createRectangleByCenterPoint(point: Point, width: number, height: number) {
    return RectangleClient.createRectangleClient(
      [point[0] - width / 2, point[1] - height / 2],
      width,
      height
    );
  },
  createRectangleClient(
    point: Point,
    width: number,
    height: number
  ): RectangleClient {
  ...
  getConnectionPoint: (rectangle: RectangleClient, point: PointOfRectangle) => {
    return [
      rectangle.x + rectangle.width * point[0],
      rectangle.y + rectangle.height * point[1],
    ] as Point;
  },
  expand(
  ...
  getGapCenter(
    rectangle1: RectangleClient,
    rectangle2: RectangleClient,
    ...
    const align = [
      rectangle1[axis],
      rectangle1[axis] + rectangle1[side],
      rectangle2[axis],
      rectangle2[axis] + rectangle2[side],
    ];
