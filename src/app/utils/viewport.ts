...
export function calcNewViewBox(board: PlaitBoard, zoom: number) {
  ...
  const viewBox = [
    ...
    elementHostBBox.right -
      elementHostBBox.left +
      (horizontalPadding * 2) / zoom,
...
export function getViewBoxCenterPoint(board: PlaitBoard) {
  ...
  return [
    childrenRectangle.x + childrenRectangle.width / 2,
    childrenRectangle.y + childrenRectangle.height / 2,
  ] as Point;
}
...
export function updateViewportOffset(board: PlaitBoard) {
  ...
  const [scrollLeft, scrollTop] = toHostPointFromViewBoxPoint(
    board,
    origination
  );
...
export function updateViewportContainerScroll(
  board: PlaitBoard,
  left: number,
  top: number,
  isFromViewportChange: boolean = true
) {
  ...
  if (
    ...
    if (
      previousScrollLeft === viewportContainer.scrollLeft &&
      previousScrollTop === viewportContainer.scrollTop
    ) {
...
export function updateViewportByScrolling(
  board: PlaitBoard,
  scrollLeft: number,
  scrollTop: number
) {
...
export function initializeViewportOffset(board: PlaitBoard) {
  ...
  if (!board.viewport?.origination && boardContainer) {
    const zoom = board.viewport.zoom;
    const viewportContainerRect = boardContainer.getBoundingClientRect();
    const viewBox = calcNewViewBox(board, zoom);
    const centerX = viewBox[0] + viewBox[2] / 2;
    const centerY = viewBox[1] + viewBox[3] / 2;
    const origination = [
      centerX - viewportContainerRect.width / 2 / zoom,
      centerY - viewportContainerRect.height / 2 / zoom,
    ] as Point;
...
export const updateViewportOrigination = (
  board: PlaitBoard,
  origination: Point
) => {
